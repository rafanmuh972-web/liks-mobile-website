(function(){
  ...
  const API_URL = window.LIKS_CHAT_API_URL || '/api/chat';
  const chatHistory = [];
  ...
  box.innerHTML = `
    <div class="lm-chat-head"><div><strong>Chat Liks Mobile</strong><small>Asisten AI katalog iPhone</small></div><button class="lm-chat-close" aria-label="Tutup">×</button></div>
  ...
  function fallbackAnswer(msg){
    ...
    if(m.includes('harga') || m.includes('price') || m.includes('katalog')) return `Harga lengkap ada di halaman <a class="lm-wa-link" href="${catalogUrl}">Katalog</a>. Kamu bisa filter Ex iBox, Ex Inter, Bea Cukai, Brand New, atau Mayday.`;
    if(m.includes('ibox') || m.includes('inter') || m.includes('bea') || m.includes('brand')) return `Kategori unit di Liks Mobile: <b>Ex iBox</b>, <b>Ex Inter</b>, <b>Bea Cukai</b>, <b>Brand New</b>, dan <b>Mayday</b>. Perbedaan utamanya ada di asal unit, kondisi, dan harga.`;
    ...
    return `Saya belum bisa menjawab detail itu. Lanjutkan ke <a class="lm-wa-link" href="${waLink('Halo Liks Mobile, saya mau konsultasi iPhone.')}" target="_blank">WhatsApp admin</a>.`;
  }
  ...
  async function handle(text){
    ...
    addMsg(`<span id="${typingId}">Sedang mengetik...</span>`,'bot');
    try{
      const reply = await askAI(text);
      const typing = document.getElementById(typingId);
      if(typing) typing.parentElement.remove();
      addMsg(cleanReply(reply));
      chatHistory.push({role:'assistant',content:reply});
    }catch(error){
      const typing = document.getElementById(typingId);
      if(typing) typing.parentElement.remove();
      const reply = fallbackAnswer(text);
      addMsg(reply);
  ...
  toggle.addEventListener('click',()=>{ box.classList.toggle('open'); if(box.classList.contains('open') && !body.dataset.started){ body.dataset.started='1'; addMsg('Halo! Saya chatbot AI Liks Mobile. Mau tanya stok, harga, kategori unit, atau cara order?'); addQuick(); }});
